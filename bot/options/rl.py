...
# Signal IC constants
SIGNAL_TYPES  = ['news', 'insider', 'ca', 'polymarket', 'contrarian', 'vwap']
IC_WINDOW     = 30
IC_STRONG     = 0.15
IC_WEAK       = 0.03
MAX_SIG_BOOST = 0.25
CONTRARIAN_BOOST_FACTOR = 0.3  # Contrarian signals get extra weight
...
class RLTrainer:
    ...
    def record_signal_context(self, trade_id: str, symbol: str, strategy: str,
                               ...
                               ca_score: float = 0.0,
                               vwap_signal_strength: float = 0.0,
        ...
        self.weights.setdefault('pending_signals', {})[trade_id] = {
            ...
            'vwap_signal_strength': round(vwap_signal_strength, 3),
            'vwap_deviation': round(vwap_deviation, 4),
            'volume_ratio': round(volume_ratio, 2),
            'entry_ts': time.time(),
        }
        logger.info('[RL] Signal ctx %s (%s): news=%.3f insider=%.3f ca=%.3f poly=%.3f contrarian=%.3f vwap=%.3f dev=%.1%% vol=%.1fx',
                          trade_id, symbol, news_score, insider_score, ca_score, polymarket_score, contrarian_score,
                          vwap_signal_strength, vwap_deviation * 100, volume_ratio)
        self._save_weights()
    ...
    def update_signal_ic(self, trade_id: str, pnl: float, strategy: str):
        ...
        for sig_type in SIGNAL_TYPES:
            # Handle VWAP signal strength vs other signal scores
            if sig_type == 'vwap':
                score = ctx.get('vwap_signal_strength', 0.0)
            else:
    ...
    def signal_kelly_adj(self, news_score: float = 0.0,
                          ...
                          polymarket_score: float = 0.0,
                          contrarian_score: float = 0.0,
                          vwap_score: float = 0.0) -> float:
        """Return Kelly multiplier (0.75..1.30) based on signal IC × current scores."""
        w   = self.weights
        adj = 0.0

        # Standard signals
        for sig_type, score in [('news', news_score), ('insider', insider_score), ('ca', ca_score), ('polymarket', polymarket_score), ('vwap', vwap_score)]:
        ...
        # Contrarian signals get enhanced weight when IC is strong
        contrarian_ic = w.get('signal_ic', {}).get('contrarian', 0.0)
        if abs(contrarian_ic) >= IC_WEAK:
            weight = min(abs(contrarian_ic) / IC_STRONG, 1.0)
            # Contrarian signals get CONTRARIAN_BOOST_FACTOR extra impact
            contrarian_adj = weight * contrarian_score * MAX_SIG_BOOST * CONTRARIAN_BOOST_FACTOR
            adj += contrarian_adj
            logger.debug('[RL] Contrarian Kelly adj: ic=%.3f score=%.3f adj=%.3f',
                        contrarian_ic, contrarian_score, contrarian_adj)

        # Cap total adjustment (slightly higher for contrarian boosts)
        max_boost = MAX_SIG_BOOST * (1.0 + CONTRARIAN_BOOST_FACTOR * 0.5)
        adj = max(-max_boost, min(max_boost, adj))
    ...
    def record_contrarian_insights(self, contrarian_data: Dict):
        """Record contrarian research insights for RL learning enhancement."""
        if not contrarian_data or 'symbols' not in contrarian_data:
            return

        # Store contrarian memory for learning reinforcement
        if not hasattr(self, 'contrarian_memory'):
            self.weights.setdefault('contrarian_memory', [])

        # Extract key patterns for learning
        insights = {
            'timestamp': contrarian_data.get('updated_at'),
            'summary': contrarian_data.get('summary', {}),
            'high_paradigm_symbols': [s['symbol'] for s in contrarian_data['symbols']
                                    if s.get('paradigm_score', 0) > 0.4],
            'high_boost_symbols': [s['symbol'] for s in contrarian_data['symbols']
                                 if s.get('boost_potential', 0) > 0.3],
            'high_risk_symbols': [s['symbol'] for s in contrarian_data['symbols']
                                if s.get('over_consensus_risk', 0) > 0.5],
            'pattern_counts': {
                'paradigm_shifts': len([s for s in contrarian_data['symbols'] if s.get('paradigm_score', 0) > 0.4]),
                'consensus_risks': len([s for s in contrarian_data['symbols'] if s.get('over_consensus_risk', 0) > 0.5]),
                'strong_divergence': len([s for s in contrarian_data['symbols'] if s.get('divergence_strength', 0) > 0.6])
            }
        }

        # Add to contrarian memory (keep last 50 insights)
        self.weights['contrarian_memory'].append(insights)
        if len(self.weights['contrarian_memory']) > 50:
            self.weights['contrarian_memory'].pop(0)

        logger.info('[RL] Contrarian insights recorded: %d paradigm, %d boost, %d risk symbols',
                   len(insights['high_paradigm_symbols']), len(insights['high_boost_symbols']),
                   len(insights['high_risk_symbols']))
        self._save_weights()

    def get_contrarian_learning_context(self, symbol: str) -> Dict[str, float]:
        """Get contrarian learning context for a symbol to enhance trade decisions."""
        if 'contrarian_memory' not in self.weights:
            return {'contrarian_score': 0.0, 'paradigm_potential': 0.0, 'consensus_risk': 0.0}

        # Analyze recent contrarian patterns for this symbol
        recent_insights = self.weights['contrarian_memory'][-10:]  # Last 10 observations
        paradigm_count = sum(1 for insight in recent_insights
                           if symbol in insight.get('high_paradigm_symbols', []))
        boost_count = sum(1 for insight in recent_insights
                         if symbol in insight.get('high_boost_symbols', []))
        risk_count = sum(1 for insight in recent_insights
                        if symbol in insight.get('high_risk_symbols', []))

        # Convert to normalized scores (0-1 range)
        total_observations = max(1, len(recent_insights))
        return {
            'contrarian_score': boost_count / total_observations,
            'paradigm_potential': paradigm_count / total_observations,
            'consensus_risk': risk_count / total_observations
        }
    ...
    def get_vwap_data(self, symbol: str) -> Dict[str, float]:
        """Get VWAP data for a symbol to enhance trade decisions."""
        try:
            ...
            if vwap_data:
                ...
                vwap_signal_strength = vwap_data.signal_strength
                ...
                return {
                    'vwap_signal_strength': round(vwap_signal_strength, 3),
                    'vwap_deviation': round(vwap_data.vwap_deviation, 3),
                    'volume_ratio': round(vwap_data.volume_ratio or 1.0, 3),
                    'vwap_bands_position': round(bands_position, 3),
                    'vwap_signal': vwap_data.vwap_signal
                }
        except Exception as e:
            logger.warning('[RL] Failed to get VWAP data for %s: %s', symbol, e)

        return {
            'vwap_signal_strength': 0.0,
            'vwap_deviation': 0.0,
            'volume_ratio': 1.0,
            'vwap_bands_position': 0.0,
            'volume_spike': 0.0
        }
